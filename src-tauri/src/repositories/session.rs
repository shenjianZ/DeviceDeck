...
impl<'a> SessionRepository<'a> {
    ...
    pub fn update_session_status(&self, id: &str, status: SessionStatus, stopped_at: Option<u64>) -> Result<(), AppError> {
        let conn = self.db.conn()?;
        conn.execute(
    ...
    pub fn get_active_sessions(&self) -> Result<Vec<MirrorSession>, AppError> {
        let conn = self.db.conn()?;
        let mut stmt = conn.prepare(
            "SELECT id, device_serial, platform, process_id, status, started_at, stopped_at, config
             FROM mirror_sessions WHERE status = 'running' ORDER BY started_at DESC",
        )?;
